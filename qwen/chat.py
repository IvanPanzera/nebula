...
from config import CONTEXT, DRAFT_MAX, RESIDENT_EXPERTS, PREFILL_BATCH
from native import Native
from verification import DEFAULT_LEVEL

ROOT=Path(__file__).resolve().parent.parent
TOKENIZER=ROOT/'assets/tokenizer'


@lru_cache(maxsize=1)
def chat_assets():
    manifest=json.loads((TOKENIZER/'manifest.json').read_text())
...
def read_turn(messages,context,thinking,backend=None):
    ...
    while True:
        ...
        if prompt.strip()=='/reset':
            ...
            print('Conversation reset.'+(' The model remains loaded.' if backend is not None else ''),file=sys.stderr)
            continue
        ...
        if len(tokens)>=context:
            print(f'The prompt requires {len(tokens)} tokens; the context capacity is {context}. '
                  'Shorten the message or use /reset to start a new conversation.',file=sys.stderr)
            continue
        messages[:]=proposed
...
def main():
    ...
    p.add_argument('--resident-experts',type=int,default=RESIDENT_EXPERTS);p.add_argument('--max-tokens',type=int,default=2048)
    ...
    p.add_argument('--verification-level',type=int,choices=range(1,4),default=DEFAULT_LEVEL,
                   help='3 exact greedy (default); 2 top-3/80%%; 1 top-10/20%% target-likelihood tolerance')
    p.add_argument('--prefill-batch',type=int,default=PREFILL_BATCH)
    p.add_argument('--storage-config',help='storage JSON (default: qwen/storage.json)')
    ...
    if not 1<=args.prefill_batch<=8192:p.error('prefill batch must be 1..8192')
    ...
    with Native(context=args.context,resident=args.resident_experts,prefill_batch=args.prefill_batch,storage_config=args.storage_config) as backend:
        while True:
            ...
            for token in generate(backend,tokens,max_new_tokens=args.max_tokens,draft_max=args.draft_max,
                                  adaptive=not args.fixed_draft,stats=stats,verification_level=args.verification_level):
            ...
            report=dict(config=vars(args),prompt_tokens=len(tokens),output_ids=generated,answer=answer,
                        decode=asdict(stats),native_before=before,native_after=backend.stats(),
                        sampler='greedy' if args.verification_level==DEFAULT_LEVEL else 'relaxed-greedy',
                        ...
                        target_experts=f'fixed top-{args.resident_experts} hotlist per layer; CPU routed MoE on handoff; omit missing current softmax mass < 0.10')
            if args.report:
            ...
            print(f"[{stats.output_tokens} token, {rate:.2f} token/s, {stats.prefill_seconds:.2f}s prefill, "
                  ...
                  f"max proposed N {stats.max_draft_used}]",file=sys.stderr)
            if at_limit:
                kind='response' if args.max_tokens<=args.context-len(tokens) else 'context'
                print(f'Reached the {kind} limit.',file=sys.stderr)
